...
class RandomGrad(Optimizer):
    def __init__(self, params, magn = 1e-5, opt = None, step_opposite = True, sampler: Callable = torch.randn_like):
        ...
        Args:
            ...
            magn (float): The magnitute of random petrubation, at smaller values gradients will be more accurate, but extremely small values don't seem to work well. Values generated by `sampler` will be multiplied by this. Defaults to 1e-5. Supports parameter groups.
        ...
        ```py
        optimizer = RandomGrad(model.parameters(), magn=1e-5, opt=optim.AdamW(MODEL.parameters(), magn=1e-3))
        ...
        approximator = RandomGrad(model.parameters(), magn=1e-5, opt=None)
        optimizer = optim.AdamW(MODEL.parameters(), magn=1e-3)
        def closure():
        ...
        if loss > self.lowest_loss:
            ...
            for group, p in foreach_group_param(self.param_groups):
                ...
                if group["step_opposite"]:
                    if p.grad is None: p.grad = state["direction"] / group["magn"]
                    else: p.grad.add_(state["direction"] / group["magn"])
...
class RandomBestGrad(Optimizer):
    ...
    def step(self, closure:Callable): # type:ignore #pylint:disable=W0222
        ...
        for group, p in foreach_group_param(self.param_groups):
            ...
            # set the gradients
            if p.grad is None: p.grad = -state["best"] / group["magn"]
            else: p.grad.add_(-state["best"] / group["magn"])
