    ...
    for dim in range(len(shape)):
        ...
        if diff > 0:
            if diff % 2 == 0:
                ...
                elif where == 'start': padding.extend([int(diff), 0])
                elif where == 'end': padding.extend([0, int(diff)])
                else: raise ValueError(f'Invalid where: {where}')
            else:
                if diff == 1:
                    if where in ('center', 'start'): padding.extend([1, 0])
                    elif where == 'end': padding.extend([0, 1])
                    else: raise ValueError(f'Invalid where: {where}')
                else:
...
class PadToSize(torch.nn.Module):
    def __init__(self, where:Literal['center', 'start', 'end'] = 'center', mode='constant', value=None):
        """Pads `input` to have the same shape as `target`.
    ...
    def forward(self, input:torch.Tensor, shape:tuple):
        return pad_to_shape(input, shape=shape, where=self.where, mode=self.mode, value=self.value)
